...
AGENT_ARCHITECTURE = [
    {
        "layer": "Planner",
        "purpose": "Classifies the business question and chooses a single-step NL2SQL flow or a multi-step diagnosis plan.",
    },
    {
        "layer": "RAG Knowledge",
        "purpose": "Retrieves schema docs, business rules, metric definitions, and SQL examples before query generation.",
    },
    {
        "layer": "Tool Router",
        "purpose": "Selects query_database, calculate_metric, generate_chart, and generate_report tools.",
    },
    {
        "layer": "SQL Sandbox",
        "purpose": "Allows read-only SELECT statements, blocks write keywords, and caps result size.",
    },
    {
        "layer": "Observability",
        "purpose": "Logs intent, latency, row count, retrieved chunks, tool count, success status, and estimated token use.",
    },
]

EVALUATION_CASES = [
    {
        "question": "查询过去三个月销量最高的三个品牌",
        "expected_intent": "rag_top_brands_by_units_sold",
        "covers": "RAG retrieval + NL2SQL + chart",
    },
    {
        "question": "Compare BMW and Tesla sales performance this year",
        "expected_intent": "rag_agent_brand_comparison",
        "covers": "Entity extraction + tool calling + metric comparison",
    },
    {
        "question": "Analyze why sales decreased in Q2 and suggest actions",
        "expected_intent": "rag_agent_sales_decline_diagnosis",
        "covers": "Agent planning + multi-query diagnosis + report",
    },
    {
        "question": "库存最多的五个品牌",
        "expected_intent": "rag_inventory_by_brand",
        "covers": "Inventory business rules + SQL Sandbox",
    },
    {
        "question": "drop table vehicles",
        "expected_intent": "blocked_by_guardrail",
        "covers": "Safety guardrail + write-operation blocking",
    },
]
...
def validate_read_only_sql(sql):
    ...
    if any(token in lowered for token in SQL_BLOCKLIST):
        raise ValueError("SQL Sandbox blocked a non-read-only or multi-statement query.")
    return cleaned


def execute_read_query(db, sql, params=()):
    cleaned = validate_read_only_sql(sql)
    sandboxed_sql = f"SELECT * FROM ({cleaned}) AS agent_safe_query LIMIT ?"
    safe_params = (*params, MAX_QUERY_ROWS)
    return [dict(row) for row in db.execute(sandboxed_sql, safe_params).fetchall()]
...
def make_tool_call(name, purpose, input_summary, output_summary, latency_ms=0, status="success"):
    return {
        "name": name,
        "purpose": purpose,
        "input": input_summary,
...
def estimate_token_use(question, retrieved_context, sql, report):
    text_parts = [question, sql, " ".join(report)]
    text_parts.extend(chunk["text"] for chunk in retrieved_context)
    total_chars = sum(len(part or "") for part in text_parts)
    ...
    return [
        {
            ...
            "status": "Active",
            "detail": "Business questions are mapped to predefined analytics intents or diagnosis workflows.",
        },
        {
            "name": "Read-only SQL Sandbox",
            "status": "Active",
            "detail": "Only SELECT statements are executed; write keywords and multi-statement SQL are blocked.",
        },
        {
            "name": "Row limit",
            "status": "Active",
            "detail": f"Database tool returns at most {MAX_QUERY_ROWS} rows per query.",
        },
        {
            "name": "Sensitive table isolation",
            "status": "Active",
            "detail": "Agent templates do not expose users, password hashes, or authentication data.",
        },
        {
            "name": "Trace logging",
            "status": "Active",
            "detail": "Each run records success, latency, retrieved chunks, tool count, rows, and estimated token use.",
        },
    ]


def ensure_agent_runs_table(db):
    db.executescript(
        """
...
def record_agent_run(db, question, result, success=True, error=None):
    ensure_agent_runs_table(db)
    observability = result.get("observability", {})
    db.execute(
        """
        INSERT INTO agent_runs (
            question, intent, success, safety_status, latency_ms, retrieved_chunks,
            tool_count, row_count, estimated_tokens, error
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            question,
            result.get("intent", "blocked_or_unknown"),
            1 if success else 0,
            observability.get("safety_status", "blocked" if not success else "passed"),
            observability.get("latency_ms", 0),
            observability.get("retrieved_chunks", 0),
            observability.get("tool_count", 0),
            observability.get("row_count", 0),
            observability.get("estimated_tokens", 0),
            error,
        ),
    )
    db.commit()
...
def attach_observability(db, question, result, started_at, success=True, error=None):
    ...
    observability = {
        ...
        "row_count": len(rows),
        "estimated_tokens": estimate_token_use(question, retrieved_context, sql, report),
        "safety_status": "passed" if success else "blocked",
        "knowledge_cache": "enabled",
    }
    result["observability"] = observability
    result["guardrails"] = build_guardrails()
    result["architecture"] = AGENT_ARCHITECTURE
    record_agent_run(db, question, result, success=success, error=error)
    return result


def get_agent_observability(db):
    ensure_agent_runs_table(db)
    summary = db.execute(
        """
        SELECT
            COUNT(*) AS total_runs,
            COALESCE(ROUND(AVG(success) * 100, 1), 0) AS success_rate,
            COALESCE(ROUND(AVG(latency_ms), 0), 0) AS avg_latency_ms,
            COALESCE(ROUND(AVG(tool_count), 1), 0) AS avg_tool_count,
            COALESCE(ROUND(AVG(retrieved_chunks), 1), 0) AS avg_retrieved_chunks,
            COALESCE(ROUND(AVG(estimated_tokens), 0), 0) AS avg_estimated_tokens
        FROM agent_runs
        """
    ).fetchone()
    ...
    intent_stats = db.execute(
        ...
        FROM agent_runs
        GROUP BY intent
        ORDER BY runs DESC, avg_latency_ms ASC
        LIMIT 8
        """
    ).fetchall()
    return {
...
def run_template_agent(db, question, retrieved_context):
    ...
    tool_calls = [
        ...
        make_tool_call(
            "sql_sandbox",
            "Validate that generated SQL is read-only and row-limited.",
            clean_sql(query.sql),
            f"SELECT-only query accepted; max rows={MAX_QUERY_ROWS}.",
        ),
        make_tool_call(
            "query_database",
...
def run_diagnosis_agent(db, question, retrieved_context):
    ...
    return result_payload(
        ...
        tool_calls=[
            ...
            make_tool_call(
                "sql_sandbox",
                "Validate diagnosis queries before execution.",
                "monthly trend, inventory, pending pipeline, Q2 brand mix",
                f"All diagnosis queries are SELECT-only; max rows={MAX_QUERY_ROWS}.",
            ),
            make_tool_call(
                "query_database",
...
def blocked_result(summary):
    return {
        ...
        "tool_calls": [
            make_tool_call(
                "sql_sandbox",
                "Block unsafe user intent before SQL generation.",
                "write-operation keyword detected",
                "request blocked",
                status="blocked",
            )
        ],
...
def run_sales_analyst_core(db, question):
    text = normalize(question)
    if not text:
        return None
    if is_unsafe_question(text):
        return blocked_result(
            "For safety, AutoOps AI Sales Analyst only supports read-only operational questions."
        )
...
def run_sales_analyst(db, question):
    ...
    try:
        result = run_sales_analyst_core(db, question)
        if result is None:
            return None
        success = not result.get("unsupported", False)
        return attach_observability(db, question, result, started_at, success=success)
    except Exception as exc:
        result = blocked_result(
            "The agent could not complete this request safely. Try a narrower sales, inventory, trend, or comparison question."
        )
