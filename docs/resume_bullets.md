...
- Designed SQL Sandbox guardrails and an observability console, blocking write-operation intent while logging success rate, latency, retrieved chunks, tool count, row count, and estimated token usage.
...
- Built an Evaluation Console with reference test cases and runtime logs to monitor intent success rate, latency, SQL Sandbox status, tool count, retrieved context, and result volume.
...
- Added SQL Sandbox guardrails and run-level observability for safety, latency, success rate, retrieved context, tool count, and estimated token usage.
